...
export const NuevaSolicitud = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.especialidad || !form.hospital) {
      toast.error('Selecciona especialidad y centro asistencial');
      return;
    }
    setLoading(true);
    try {
      ...
      <div className="max-w-lg">
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="px-6 py-4 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Centro asistencial</label>
              <select
                name="hospital"
                value={form.hospital}
                onChange={handleChange}
              ...
              >
                ...
                {CENTROS_ASISTENCIALES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
